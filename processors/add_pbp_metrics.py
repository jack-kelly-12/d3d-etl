...
def add_run_expectancy(df: pd.DataFrame, re_lookup: dict) -> pd.DataFrame:
    ...
    df['re_before'] = df.apply(
        ...
        if pd.notna(r['bases_before']) and pd.notna(r['outs_before']) else pd.NA,
        axis=1
    )
...
def process_division(data_dir: Path, division: int, year: int) -> pd.DataFrame:
    ...
    metric_cols = ['woba', 're_before', 're_after', 're_delta', 'rea',
                   'home_win_exp_before', 'home_win_exp_after', 'delta_home_win_exp',
                   'wpa', 'li', 'wpa_li', 'score_diff_before', 'score_diff_after',
                   'times_through_order', 'high_leverage_fl', 'low_leverage_fl']
...
def keep_columns(df: pd.DataFrame) -> pd.DataFrame:
    return df[['year', 'date', 'contest_id', 'inning', 'away_score', 'home_score', 'division',
    'home_team_id', 'away_team_id', 'is_neutral_site', 'half', 'play_description', 'play_id',
    'away_team_name', 'home_team_name', 'bat_team_id', 'pitch_team_id', 'bat_team_name',
    'pitch_team_name', 'game_end_fl', 'inn_end_fl', 'new_game_fl', 'new_inn_fl', 'sub_fl', 'sub_in',
    'sub_out', 'sub_pos', 'int_bb_fl', 'sh_fl', 'sf_fl', 'top_inning_fl', 'pitcher_sub_fl',
    'outs_on_play', 'outs_before', 'outs_after', 'runs_on_play',
    'away_score_before', 'home_score_before', 'home_score_after', 'away_score_after', 'runs_this_inn',
    'runs_roi', 'batter_name', 'r1_name', 'r2_name', 'r3_name', 'bases_before',
    'r1_after', 'r2_after', 'r3_after', 'bases_after', 'bat_order', 'event_type', 'batted_ball_type',
    'pitcher_name', 'pitcher_id', 'batter_id', 'r1_id', 'r2_id', 'r3_id', 'player_name', 'player_id',
    'woba', 're_before', 're_after', 're_delta', 'rea', 'score_diff_before', 'score_diff_after',
    'home_win_exp_before', 'li', 'home_win_exp_after', 'delta_home_win_exp', 'wpa', 'wpa_li',
    'times_through_order', 'high_leverage_fl', 'low_leverage_fl']]
