...
def fill_pitcher_names(pbp_df: pd.DataFrame, pitching_lineups: pd.DataFrame) -> pd.DataFrame:
    ...
    for (contest_id, team_id), group in pitching.groupby(["contest_id", "team_id"]):
        pitcher_queue[(contest_id, team_id)] = list(
            zip(group["player_name"].tolist(), group["player_id"].tolist(), strict=True)
        )
    ...
    for i, row in df.iterrows():
        ...
        if is_pitcher_sub or key not in current_pitcher:
            ...
            if idx < len(queue):
                ...
                current_pitcher[key] = (
                    sub_in if is_pitcher_sub and sub_in else current_pitcher.get(key, "")
                )
...
def match_player_in_game(
    name: str, contest_id, team_id, game_lookups: dict, full_lookup: dict, threshold: int = 70
) -> tuple[str, str | None]:
...
def match_players_with_lineups(
    df: pd.DataFrame,
    player_col: str,
    team_col: str,
    contest_col: str,
