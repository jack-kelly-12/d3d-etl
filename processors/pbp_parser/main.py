...
def parse_pbp(pbp: pd.DataFrame, team_history: pd.DataFrame, year: int,
              batting_lineups: pd.DataFrame, pitching_lineups: pd.DataFrame,
...
def parse_pbp_runs(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["away_score_before"] = score_before(df["game_end_fl"], df["runs_on_play"], df["half"], home_team=0)
    df["home_score_before"] = score_before(df["game_end_fl"], df["runs_on_play"], df["half"], home_team=1)
    df["home_score_after"] = df["home_score_before"] + np.where(df.half == "Bottom", df.runs_on_play, 0)
    df["away_score_after"] = df["away_score_before"] + np.where(df.half == "Top", df.runs_on_play, 0)
...
def parse_pbp_outs(df: pd.DataFrame) -> pd.DataFrame:
    df["outs_on_play"], df["outs_on_play_reason"] = outs_on_play(df["p1_text"], df["p2_text"], df["p3_text"], df["p4_text"])
...
def load_data(data_dir: Path, division: int, year: int) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    if team_history.empty:
        print(f"Warning: No team history found at {team_history_path}")
    ...
    if batting_lineups.empty:
        print(f"Warning: No batting lineups found at {batting_path}")
    if pitching_lineups.empty:
        print(f"Warning: No pitching lineups found at {pitching_path}")
    if roster.empty:
        print(f"Warning: No roster found at {roster_path}")
...
def main(data_dir: str, year: int, divisions: list[int]):
    ...
    for division in divisions:
        ...
        try:
            pbp, team_history, batting_lineups, pitching_lineups, roster = load_data(data_dir, division, year)
        except FileNotFoundError as e:
            print(f"{e}, skipping division {division}")
            continue
