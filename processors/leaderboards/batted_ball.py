...
def add_batted_ball_flags(df: pd.DataFrame) -> pd.DataFrame:
    ...
    batter_hand = (
        df.get("batter_hand", pd.Series(index=df.index, dtype="object")).fillna("").astype(str)
    )
    pitcher_hand = (
        df.get("pitcher_hand", pd.Series(index=df.index, dtype="object")).fillna("").astype(str)
    )
    ...
    right_pattern = "|".join(
        [FIELD_PATTERNS["to_rf"], FIELD_PATTERNS["to_1b"], FIELD_PATTERNS["to_2b"]]
    )
    left_pattern = "|".join(
        [FIELD_PATTERNS["to_lf"], FIELD_PATTERNS["to_3b"], FIELD_PATTERNS["to_ss"]]
    )
    ...
    pull_sw = has_pitch & ((to_right & (pitcher_hand == "R")) | (to_left & (pitcher_hand == "L")))
    oppo_sw = has_pitch & ((to_right & (pitcher_hand == "L")) | (to_left & (pitcher_hand == "R")))
...
def calculate_batted_ball_stats(df: pd.DataFrame, type="batter") -> pd.DataFrame:
    ...
    stats["pull_air_pct"] = np.where(
        total_dir > 0, pull_air.reindex(stats.index, fill_value=0) / total_dir * 100, np.nan
    )
    stats["oppo_gb_pct"] = np.where(
        total_dir > 0, oppo_gb.reindex(stats.index, fill_value=0) / total_dir * 100, np.nan
    )
