...
def calculate_batting_metrics(group: pd.DataFrame, weights: dict) -> pd.Series:
    ...
    outs = (group['event_type'] == EventType.GENERIC_OUT).sum() + \
           (group['event_type'] == EventType.STRIKEOUT).sum()
    ...
    if pa == 0:
        return pd.Series({
            'woba': np.nan, 'ba': np.nan, 'pa': 0,
            'rea': 0, 'slg': np.nan, 'obp': np.nan
        })
    ...
    return pd.Series({
        'woba': woba, 'ba': ba, 'pa': pa,
        'rea': rea, 'slg': slg, 'obp': obp
    })


def add_handedness(pbp: pd.DataFrame, info: pd.DataFrame,
                   ...
                   out_batter_hand: str = 'batter_hand') -> pd.DataFrame:
    throws_map = (
    ...
    pbp[out_batter_hand] = pbp[out_batter_hand].where(pbp[out_batter_hand].isin(["R", "L", "S", "B"]))
...
def filter_by_team_history(df: pd.DataFrame, team_history: pd.DataFrame) -> pd.DataFrame:
    ...
    valid_teams['division'] = pd.to_numeric(valid_teams['division'], errors='coerce').astype('Int64')
