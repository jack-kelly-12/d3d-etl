...
def calculate_batting_value(df: pd.DataFrame, runs_per_win: float) -> tuple[pd.DataFrame, pd.DataFrame]:
    batting = df[~df['batter_id'].isna()].copy()

    player_stats = batting.groupby('batter_id').agg({
        'batter_name': 'first',
        'bat_team_name': 'first',
        'bat_team_id': 'first',
        'wpa': 'sum',
    ...
    team_stats = batting.groupby('bat_team_id').agg({
        'bat_team_name': 'first',
        'wpa': 'sum',
        'rea': 'sum',
        'wpa_li': 'sum',
    ...
    player_cols = ['player_id', 'player_name', 'team_name', 'team_id',
                   'wpa', 'neg_wpa', 'pos_wpa', 'rea', 'rew', 'pli', 'wpa_li', 'clutch']
    team_cols = ['team_id', 'team_name', 'wpa', 'neg_wpa', 'pos_wpa', 'rea', 'rew', 'pli', 'wpa_li', 'clutch']

    return player_stats[player_cols], team_stats[team_cols]


def calculate_pitching_value(df: pd.DataFrame, runs_per_win: float) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    player_stats = pitching.groupby('pitcher_id').agg({
        'pitcher_name': 'first',
    ...
    team_stats = pitching.groupby('pitch_team_id').agg({
        'pitch_team_name': 'first',
        'pwpa': 'sum',
        'prea': 'sum',
        'pwpa_li': 'sum',
    ...
    pitching_changes = pitching[(pitching['sub_fl'] == 1) & (pitching['sub_pos'] == 'p')].groupby('pitch_team_id').size()
    team_stats['pitching_changes'] = team_stats['pitch_team_id'].map(pitching_changes).fillna(0).astype(int)
    ...
    player_cols = ['player_id', 'player_name', 'team_name', 'team_id',
                   'wpa', 'neg_wpa', 'pos_wpa', 'rea', 'rew', 'pli', 'wpa_li', 'clutch']
    team_cols = ['team_id', 'team_name', 'wpa', 'neg_wpa', 'pos_wpa', 'rea', 'rew',
                 'pli', 'wpa_li', 'clutch', 'pitching_changes']
