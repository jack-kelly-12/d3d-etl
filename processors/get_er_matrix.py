...
def get_expected_runs_matrix_2(bases_before, outs, runs_rest_of_inn):
    er = pd.DataFrame(
        {"bases_before": bases_before, "outs": outs, "runs_rest_of_inn": runs_rest_of_inn}
    ).dropna()

    er = (
        er.groupby(["bases_before", "outs"])
        .agg(
    ...
    er_matrix = pd.DataFrame(
        er_matrix,
        index=["NNN", "YNN", "NYN", "YYN", "NNY", "YNY", "NYY", "YYY"],
        columns=["0", "1", "2"],
    )
...
def main(data_dir, year, divisions=None):
    ...
    for division in divisions:
        try:
            ...
            logger.error("Error processing %s: %s", division_year_label(division, year), str(e))
            continue
    ...
    if not final_dfs:
        logger.warning("No matrices were generated!")
        return None
