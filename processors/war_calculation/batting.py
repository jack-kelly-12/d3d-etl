...
def add_runner_dests_from_shift(pbp_df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["r1_dest"] = [
        dest(r, a, b, c) for r, a, b, c in zip(df["r1_id"], r1n, r2n, r3n, strict=True)
    ]
    df["r2_dest"] = [
        dest(r, a, b, c) for r, a, b, c in zip(df["r2_id"], r1n, r2n, r3n, strict=True)
    ]
    df["r3_dest"] = [
        dest(r, a, b, c) for r, a, b, c in zip(df["r3_id"], r1n, r2n, r3n, strict=True)
    ]
    return df
...
def calculate_webt(pbp_df: pd.DataFrame, runs_out: float) -> pd.DataFrame:
    ...
    for t in ["13", "2h", "1h"]:
        ...
        webt += (succ - lg_rates[t]["succ"] * opp) * 1.0 + (outc - lg_rates[t]["out"] * opp) * (
            -runs_out
        )
