...
def calculate_baserunning_constants(pbp_df: pd.DataFrame) -> dict:
    ...
    runs_out = safe_divide(pbp_df["runs_on_play"].sum(), pbp_df["outs_on_play"].sum())
...
def calculate_guts_constants(division: int, year: int, data_dir: Path) -> dict | None:
    ...
    try:
        ...
        logger.warning("Missing file for %s: %s", division_year_label(division, year), e)
        return None
    except Exception as e:
        logger.error(
            "Error calculating constants for %s: %s", division_year_label(division, year), e
        )
        return None
...
def main(data_dir: str, year: int, divisions: list[int] = None):
    ...
    for division in divisions:
        ...
        if constants:
            ...
            logger.info("Calculated %s constants", division_year_label(division, year))

    if not new_constants:
        logger.warning("No constants calculated")
        return
    ...
    logger.info("Saved %s rows to %s", len(new_constants), guts_file)
